...
把图像摊平成一长串数字后交给 MLP，当然也能完成任务；但 MLP 的结构并不知道：出现在左上角和右下角的同一条边缘，本质上是同一种局部图案。它只能靠训练数据自己摸索这层关系。**CNN**（convolutional neural network，卷积神经网络）则预先带着两条适合图像的默认假设（这叫归纳偏置）：有用的线条、角点和纹理通常只和附近像素有关；同一种局部图案即使换了位置，仍值得用同一套规则识别。
