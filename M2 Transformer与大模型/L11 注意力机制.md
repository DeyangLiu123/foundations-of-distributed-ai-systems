...
先只看一个 head，并省略 batch 维。输入是 $X\in\mathbb{R}^{S\times d}$。模型要学三个不同的投影矩阵：
...
其中 $Q,K,V\in\mathbb{R}^{S\times d_{head}}$，**$d_{head}$**（单个 attention head 的维度）是每个头处理的特征宽度。这里已经可以排除一个常见误解：$Q$、$K$、$V$ 都由同一个 $X$ 生成，但它们乘的是三组不同参数，==所以绝不是 $Q=K=V$==。
...
一组 QKV 只会产生一张权重图。**multi-head attention (MHA)**（多头注意力）把 hidden size $d$ 分成 $h$ 个子空间，每个 **attention head**（注意力头）有自己的 QKV 投影，于是能学到不同的匹配方式。入门时可以想成“一个头关注语法关系，一个头关注代词指代”；但这只是直觉，训练并不会为每个头颁发固定的人类语义标签。
