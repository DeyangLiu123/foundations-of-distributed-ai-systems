...
> [!quote]
> A **Mixture-of-Experts (MoE)** layer replaces the dense FFN with $E$ **experts**. For every token, a **router/gating network** selects the **top-k** experts and combines their outputs, so only a **sparse activation** of the model is evaluated. The system must maintain **load balancing** and an expert **capacity factor**; otherwise **token dropping** or communication hotspots appear when experts are placed with **expert parallelism** and connected by **all-to-all**.
>
> （改写自 Switch Transformers、Mixtral of Experts 与 DeepSeek-V3 Technical Report 的典型表述；不是逐字引文）

这段话把 MoE 论文最常见的一串词压在了一起。这里的 “only a sparse activation” 不是说参数被删掉，而是说==同一个 token 只走一小部分参数==；“capacity” 也不是显存容量，而是某个 expert 在一个 batch 里最多接收多少 token。读完本课再回来看，你应该能解释每个词，也能看见它们为什么最终变成网络里的 all-to-all。

## 一、从 dense FFN 到稀疏激活：先把容量和每步算力拆开

[[L12 Transformer全解剖]] 里的一层 Transformer block 可以粗略写成：attention 保持全体 token 的交互，FFN 对每个位置独立做升维、激活、降维。**dense model**（稠密模型）就是每个 token 都经过同一套 FFN 参数；参数表和每 token 的计算量一起增长。
...
> [!tip] 直觉：座位多，不等于每个人都坐满
> 671B 是剧院的总座位数，37B 是一个观众进场时真正坐到的座位数。前者决定要建多大的剧院、存多少座位；后者决定这位观众走动时要付多少步行成本。

### 1.1 路由器到底在做什么

给某一层输入 token 的 hidden state $h_t\in\mathbb{R}^{d}$。**router/gating network**（路由器/门控网络）通常是一个很小的线性层，它为 $E$ 个 expert 产生分数：

$$
r_t=W_r h_t+b_r\in\mathbb{R}^{E}.
$$

将分数变成可比较的权重（常见做法是 softmax），再取最大的 $k$ 个索引：
...
这一步叫 **top-k routing**（top-k 路由）。它不是一个“数学专家/代码专家”的人工分类器：router 只根据当前表示学习出分数，expert 的分工通常是训练中涌现的，事后未必能用人类标签解释。
...
理想情况是每个 expert 接收差不多多的 token，所有 GPU 都在忙。现实里 router 可能偏爱某几个 expert：它们的队列爆满，其他 expert 却空闲。于是 MoE 的“只算 $k$ 个 expert”并不自动等于“吞吐提升”。

### 2.1 load balancing：不要让热门车间堵死

**load balancing**（负载均衡）要让两件事同时接近均匀：

1. 实际分到每个 expert 的 token 数；
2. router 给每个 expert 的概率质量。

经典方法给训练目标加一个 **auxiliary loss**（负载均衡辅助损失），惩罚“token 比例”和“router 概率”与均匀分布偏离。它通常不是主任务 loss，而是一个带系数的正则项：主任务仍然决定“答案对不对”，辅助项提醒 router “别把所有 token 都送到同一个车间”。辅助项太强会牺牲专家选择质量，太弱则会出现热点，这是一个实际的权衡。
...
当 token 被选中但目标 expert 已达到上限，常见处理是 **token dropping**（token 丢弃）：该 token 跳过这次 expert 计算，依实现可能只保留 residual 或采用其他降级路径。它能让张量形状规整、通信可控，但会损失部分信息，尤其在训练早期或负载严重倾斜时明显。
...
3. **combine**：把输出按照 token 的原始位置和权重合并，再发回 token 的来源 rank。

因此通常有两次 **all-to-all**（全对全通信）：第一次“去专家”，第二次“回来源”。它和 [[L36 集合通信原语]] 里规则、同形状的 all-reduce 很不一样：消息目的地由 router 的数据依赖决定，热门 expert 还会制造不均匀消息和队列。
...
网络视角下，通信量主要跟“这一层有多少 token、每个 token 的 hidden size、数据精度和 top-k”有关，而不是只跟总参数量有关。top-2 会让一个 token 最多产生两份 dispatch；跨节点时还要付消息启动、重排和拥塞代价。按 [[03 约定与符号]] 的课内口径，NDR 端口是 400 Gb/s、即每方向 50 GB/s，而 H100 SXM 的 NVLink 双向合计 900 GB/s、每方向约 450 GB/s；这约 9 倍的单向差距解释了为什么 TP 常尽量留在机箱内，而 EP 的跨节点流量必须精心规划。真正的 EP 调度、通信拓扑和 overlap 见 [[L46 专家并行与MoE训练]]。
...
| **DeepSeek-V3** | 256 路由 expert + 1 个 shared expert；细粒度专家 | top-8（另过 shared） | 总参数 671B、每 token 激活 37B；同时使用 auxiliary-loss-free 负载均衡。 |
...
> [!example] 算一算：同一个 671B，为什么算力像 37B、显存却像 671B
> **① 先算权重显存（只算 BF16 权重）**
>
> $$
...
> $$
>
> 所以可以说“每 token 的主 FFN 计算约省 18×”，但不能说真实端到端 decode 一定快 18×：attention、router、通信、内存读取和 kernel 利用率都还在账上。反过来，active=37B 也绝不等于只需存 37B 权重。
...
这也是 [[L16 Scaling Law与算力账]] 中“active 参数和 total 参数必须分开报”的原因。训练 FLOPs、推理每 token FLOPs 主要看 active 路径；放置显存、checkpoint 体积、专家并行规模则要看 total 参数。读到“671B model”时，先问作者报的是哪一个；如果只给一个数字，往往还不够做系统比较。
...
2. **“A router selects the top-k experts and combines their outputs.”** router 用小线性层给 expert 打分，top-k 选索引和权重，再按权重把选中 FFN 的输出相加；这对应本课第二节的数据流图。
3. **“Only a sparse activation of the model is evaluated.”** 一次 token 只激活少数 expert，因此 active parameters 小于 total parameters；DeepSeek-V3 的课内数字是 37B 对 671B。
...
| expert | 专家 | MoE 中的一份独立 FFN 参数块，接收被路由来的 token。 |
...
| sparse activation | 稀疏激活 | 模型总参数很多，但一次 token 只执行其中一小部分。 |
| active parameters | 激活参数 | 某个 token 的前向路径实际使用的参数量，决定主计算量。 |
| total parameters | 总参数 | 模型所有权重的总量，决定容量和主要权重显存。 |
| load balancing | 负载均衡 | 让 token 和 router 概率在各 expert 之间尽量均匀。 |
| auxiliary loss | 辅助损失 | 促使 router 均衡分配 token 的额外训练目标。 |
| auxiliary-loss-free | 无辅助损失 | 通过调整 router bias 平衡负载而不直接加入辅助 loss 的策略。 |
| capacity factor | 容量因子 | 相对平均 token 分配量设置 expert 接待上限的倍率 $c$。 |
| token dropping | token 丢弃 | expert 缓冲区满时跳过该 token 的 expert 计算的处理策略。 |
| dropless | 不丢 token | 即使路由不均也处理每个 token，以动态容量换取质量和峰值资源。 |
...
| dense model | 稠密模型 | 每个 token 都激活同一套全部模型参数的模型。 |
...
> 4. 太小会溢出、增加 token dropping 或阻塞；太大会浪费 buffer 和通信峰值。token dropping 允许溢出 token 降级处理，dropless 则动态扩容/调度以保证每个 token 都经过选定 expert。
