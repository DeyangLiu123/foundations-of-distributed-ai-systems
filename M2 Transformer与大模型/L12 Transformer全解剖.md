...
>
> 想先从全局看 residual stream？打开 [[实践代码/L12_transformer_stream.html|全局 Stream 执行 DAG]]：矩阵是数据节点，RMSNorm、MatMul、Softmax 和 Residual Add 是算子节点，箭头明确标出 `token_ids + E → Embedding Lookup → embedding X`、Q/K/V 分叉、两次 residual skip 和 SwiGLU 汇合；点击任一矩阵节点仍可展开完整数值矩阵。
