...
图形只是心智模型：真实曲线会有 batch 噪声、评测间隔和数据阶段切换。学习率 schedule、warmup 与 cosine decay 的概念已在 [[L06 优化器]] 中介绍；预训练规模变大后，最重要的是同时记录 token 进度、学习率、gradient norm、吞吐、显存、通信等待和 checkpoint 版本。
...
>
> 这是理想的主计算估算。真实 wall-clock 还会受到通信、输入 I/O、评测、checkpoint、重启和长上下文阶段的影响；公开材料把 Llama 3 405B 的主训练概括为约 54 天量级，差异正好提醒我们：报告中的“有效 TFLOPS”“主训练时长”和“完整项目日历”不是同一个指标。这里应报告假设和数量级，而不是倒推出一个虚假的精确天数。
...
- **数据供给**：shard 读取、解压、token packing 和 prefetch 必须跟上 GPU。存储带宽不足时，监控里会看到 data wait 增加，而不是 GEMM 本身变慢。
...
> 7. 集群有效 FLOPS 为 $16384\times400\times10^{12}=6.5536\times10^{18}$；每 token 为 $6\times405\times10^9=2.43\times10^{12}$ FLOPs，因此约 $2.7\times10^6$ tokens/s。每天约 $2.3\times10^{11}$ token，$15.6\times10^{12}/2.3\times10^{11}\approx68$ 天。完整 wall-clock 还会受到通信、I/O、评测、checkpoint、重启和阶段切换的影响。
